...
const navItems = [
  ...
  { href: "/dashboard/tag-generator", label: "Tag Generator", icon: Tag, isPrimary: false },
  { href: "/dashboard/new-audit", label: "Steam Page Analysis", icon: Rocket, isPrimary: false },
  { href: "/dashboard/credits", label: "Buy Credits", icon: CreditCard, isPrimary: false },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      if (!mounted) return;
      setCreditsBalance(Number.isFinite(balancePayload.balance) ? Number(balancePayload.balance) : 0);
    };
      ...
      <main className={[
        ...
        <div className="mb-4 flex justify-end">
          <Link
            ...
            className={[
              "inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-[0.15em] transition-all",
              theme === "dark"
                ? "border-blue-600/40 bg-blue-600/10 text-blue-300 hover:border-blue-600/60 hover:bg-blue-600/20"
                : "border-blue-400/50 bg-blue-100 text-blue-700 hover:border-blue-500 hover:bg-blue-200",
            ].join(" ")}
          >
            Credits
            <span className={[
              "rounded-full px-2 py-0.5",
              theme === "dark" ? "bg-blue-500/20 text-blue-200" : "bg-blue-200 text-blue-800",
            ].join(" ")}>{creditsBalance}</span>
          </Link>
        </div>
